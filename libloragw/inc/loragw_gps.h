...
struct tref {
    ...
    struct timespec utc_acc;    /*!> estimated UTC time accuracy (from GPS) */
...
struct coord_s {
    ...
    float   eha;    /*!> estimated horizontal accuracy (m) */
    float   eva;    /*!> estimated vertical accuracy (m) */
    int     nsv;    /*!> number of satellites used in the position fix */
